...
// In-Memory sync data table for rapid cross-device synchronization
...
export function loadSyncDataFromDisk() {
  try {
    if (fs.existsSync(DB_FILE)) {
      ...
      console.log(`[SYNC STORE LOADED] Restored sync states for ${Object.keys(userSyncStore).length} accounts.`);
    }
  } catch (err) {
    console.warn('[SYNC STORE LOAD WARNING]', err.message);
  }
}
...
export function saveSyncDataToDisk() {
  try {
    ...
    console.warn('[SYNC STORE SAVE WARNING]', err.message);
  }
}
...
export function getDefaultUserState(user) {
  ...
  return {
    ...
    campaignConfig: {
      ...
      senderEmail: 'm4verickjack@gmail.com',
      senderName: 'Maverick Jack',
      htmlContent: ''
    },
    smtpConfig: {
      mode: 'gmail',
      user: 'shaptsevjkonikevich@gmail.com',
      pass: 'smjpsmbbqhjvovcp'
    },
    recipients: [
    ...
    apiKeys: [
      {
        ...
        key: `sk_live_sendaat_1001_99a4x28190c`,
...
export function getUserSyncData(userId, userFallback = null) {
  ...
  return userSyncStore[cleanId];
}

export function updateUserSyncData(userId, delta, deviceId = 'unknown_device', clientVersion = null) {
  ...
  // Check version divergence for optimistic concurrency
  if (clientVersion && clientVersion < serverVersion) {
    conflictDetected = true;
    console.log(`[SYNC CONFLICT DETECTED] Client v${clientVersion} vs Server v${serverVersion} for user ${cleanId}`);
  }

  // Create project / state snapshot before applying delta if significant fields changed
  ...
  // Deep-merge delta fields safely
  ...
  // Broadcast real-time update to all active devices
